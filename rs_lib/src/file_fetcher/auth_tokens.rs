...
impl AuthDomain {
  pub fn matches(&self, specifier: &Url) -> bool {
...
mod tests {
  ...
  fn test_auth_token() {
    ...
    let fixture = Url::parse("https://deno.land/x/mod.ts").unwrap();
    assert_eq!(
      auth_tokens.get(&fixture).unwrap().to_string(),
      "Bearer abc123"
    );
    let fixture = Url::parse("https://www.deno.land/x/mod.ts").unwrap();
    assert_eq!(
      auth_tokens.get(&fixture).unwrap().to_string(),
  ...
  fn test_auth_tokens_multiple() {
    ...
    let fixture = Url::parse("http://example.com/a/file.ts").unwrap();
    assert_eq!(
      auth_tokens.get(&fixture).unwrap().to_string(),
  ...
  fn test_auth_tokens_port() {
    ...
    let fixture = Url::parse("https://deno.land/x/mod.ts").unwrap();
    assert_eq!(auth_tokens.get(&fixture), None);
    let fixture = Url::parse("http://deno.land:8080/x/mod.ts").unwrap();
    assert_eq!(
      auth_tokens.get(&fixture).unwrap().to_string(),
      "Bearer abc123".to_string()
    );
