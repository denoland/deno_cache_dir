...
pub fn url_to_filename(url: &str) -> Result<String, JsValue> {
  ...
  deno_cache_dir::url_to_filename(&url)
...
pub struct GlobalHttpCache {
  cache: deno_cache_dir::GlobalHttpCache<RealSys>,
}
...
impl GlobalHttpCache {
  pub fn new(path: &str) -> Self {
    Self {
      cache: deno_cache_dir::GlobalHttpCache::new(RealSys, PathBuf::from(path)),
    }
  }
...
pub struct LocalHttpCache {
  cache: deno_cache_dir::LocalHttpCache<RealSys>,
}
...
impl LocalHttpCache {
  pub fn new(
    ...
    console_error_panic_hook::set_once();
    let global = deno_cache_dir::GlobalHttpCache::new(
      RealSys,
      wasm_string_to_path(global_path),
    );
    let jsr_url = RealSys
      .env_var("JSR_URL")
      .ok()
    ...
    let local = deno_cache_dir::LocalHttpCache::new(
